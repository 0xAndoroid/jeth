...
//! every witness node in-guest. See `set_trusted_digests` for the soundness
...
impl SparseState {
    ...
    /// read by SLOAD through `storage()`. Memo misses (slots of created accounts,
...
impl StatelessTrie for SparseState {
    ...
    fn calculate_state_root(&mut self, state: HashedPostState) -> Result<B256, StatelessTrieError> {
        ...
        // `state.accounts` is a foldhash HashMap — advice calls
        ...
        for (hashed_address, account) in accounts {
            ...
            let storage_root = match state.storages.get(&hashed_address) {
                ...
                Some(storage) => {
                    ...
                    // (insert-before-remove preserved)
                    for &(hashed_key, value) in &slots {
                        if value.is_zero() {
