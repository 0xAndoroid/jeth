...
// jeth vendor patch: the guest binary provides this hook, routing ecrecover
// through the Jolt secp256k1 inline (k256-exact semantics — see
// `jeth_core::crypto`). Returns 1 and writes `keccak(pubkey)` to `out` on
// success, 0 on recovery failure. Measured: software k256 costs ~1.5M trace
// rows per authorization vs ~230k for the inline (block 25698070 spent 396M
// rows / 16% of the block here).
#[cfg(target_arch = "riscv64")]
unsafe extern "C" {
    fn jeth_ecrecover_prehash(sig: *const u8, recid: u8, msg: *const u8, out: *mut u8) -> u8;
}
...
impl SignedAuthorization {
    ...
    pub fn recover_authority(&self) -> Result<Address, crate::error::Eip7702Error> {
        ...
        // jeth vendor patch: Jolt guest builds recover via the secp256k1
        // inline. `y_parity <= 1` is guaranteed by `signature()` above; the
        // high-s check matches upstream. The inline mirrors k256's acceptance
        // set exactly (r/s canonical nonzero, on-curve R, identity -> failure).
        #[cfg(target_arch = "riscv64")]
        {
            let mut sig = [0u8; 64];
            sig[..32].copy_from_slice(&signature.r().to_be_bytes::<32>());
            sig[32..].copy_from_slice(&signature.s().to_be_bytes::<32>());
            let msg = self.inner.signature_hash();
            let mut out = [0u8; 32];
            let ok = unsafe {
                jeth_ecrecover_prehash(
                    sig.as_ptr(),
                    self.y_parity(),
                    msg.as_ptr(),
                    out.as_mut_ptr(),
            ...
            if ok == 0 {
                return Err(crate::error::Eip7702Error::Signature(
                    alloy_primitives::SignatureError::FromBytes("secp256k1 recovery failed"),
                ));
            }
            return Ok(Address::from_slice(&out[12..]));
        }
