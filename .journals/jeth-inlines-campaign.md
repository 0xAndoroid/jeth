...
| lane | branch | task | status |
|---|---|---|---|
| B-bn254 | inlines-b-bn254 @ 419da11 | d65966f1 done (MULQ 273 / SOPQ2 415 / FP2MULQ 797 rows; pairing −20%, ecmul −5.6%; set −35.3M rows → 13.653998) | review 60ff56ad APPROVE (4 should-fix: direct seq-vs-ark tests, assert! not debug_assert, edge ≥q fix, PROVING GATE observation) → fixed a0e6462 (tests/assert only, rows identical), rebased, ff-merged into inlines-b @ f1d0fa3 (set 4,383,311,306 = 13.653998) |
...
- 04:15 PROOF GATE PASS on all three families (ac12b992; jolt-inlines-b @ c0e9fb845f + 4349e92dbb pushed to private): bn254 pairing prove 174 s / verify 0.6 s (2^24, 7.8 GiB), BLS G2 70 s, BLAKE2F ≈1 s; outputs == native. Banked in RESULTS.md gates. FINDING: BLS is_fq2 guard runs at runtime (memcmp 627 + memcpy 84 rows/Fq2 mul) → Fp2 fused op saves ~0 in that guest; bn254 guard folds. BLS lane resumed (546e8ca6) to const-fold + re-measure (G2MSM −13.9% claim to be re-verified). Card 340 done.
- 04:36 BLS guard fix 92a1093: const-fn guard; G2MSM −13.9% → −18.2%, pairing −13.0% → −17.7%, POINTEVAL −12.5% → −16.5%; Aztec −3.15% → −3.58% (1,375,135,731). Reason the old numbers were still negative: in the jeth guest the guard cost ≈103 rows/Fq2 mul (word-wise memcmp), not 711 as in the example guest. Round-2 review spawned.
- 04:43 round-2 review 3531a2f3 APPROVE (const guard sound; const-eval checked on Fq2/Fq12/bn254/bls12-377 types). Merged 92a1093 into inlines-b. Follow-up N2 delegated to 546e8ca6: same const treatment for the bn254 Fq2 guard (currently folds only by LLVM's grace).
- 05:00 bn254 guard const (bf7f1d0, rows unchanged, objdump-proven) merged into inlines-b pending round-3 review 9325039e; report agent spawned (fable-medium).
- 05:05 round-3 review 9325039e APPROVE (bn254 const guard). inlines-b @ 8327e6b force-pushed (journal commits were rebased over lane commits); PR #3 body updated with the proof gate + post-fix BLS numbers (via gh api — gh pr edit lacks read:org scope). Waiting on the report agent 7c252e4e.
- 05:40 DONE. Report https://me.andrew.ee/reports/jeth-inlines-campaign-2026-09.html (lint 0/0). Finding from the report pass: the study's POINTEVAL vector was degenerate; a valid KZG proof is 757 → 632 c/g — POINTEVAL is now the worst precompile bound (banked in RESULTS.md). Total spend ≈ $172. Cards: lanes + program → human_review (PRs #2, #3 await review). Follow-ups: BLS FP2SQR, BLAKE2b FULL20, p256 sdk limb compares, POINTEVAL-specific work (2 pairings on BLS Fp2).
